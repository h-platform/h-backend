...
module.exports = function(config){
  return {
    ...
    handlePostRequest: function(request, reply){
      client.act({ role:'database', model: 'post', cmd:'insertRecord', record: request.payload}, function (err, result) {
          if(err) {
              console.log('************** error');
    ...
    handleDeleteRequest: function(request, reply){
      client.act({ role:'database', model: 'post', cmd:'deleteRecord', record: request.payload}, function (err, result) {
        if(err) {
            console.log('************** error');
            console.log(err.details.orig$);
            reply.view('500',{msg: ''});
    ...
    handlePutRequestForId: function(request, reply){
      client.act({ role:'database', model: 'post', cmd:'updateRecord', record: request.payload, id:request.params.id}, function (err, result) {
          if(err) {
              console.log('************** error');
              console.log(err.details.orig$);
              reply.view('500',{msg: ''});
